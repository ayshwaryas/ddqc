...
write("project,tissue,cluster,cell.type,n.cells,percent.unique,genes.mean,mito.mean,markers", "result.csv")
for (project in dir(results.dir,  pattern="mc_", recursive=FALSE)) {
  for (tissue in dir(paste0(results.dir, project, "/"), recursive=FALSE)) {
    ...
    rownames(mad.cells) <- mad.cells$cell
    mad.clusters <- read.csv(paste0(mad.dir, "!clusters.csv"))
    rownames(mad.clusters) <- mad.clusters$cluster
    
    cutoff.cells <- as.character(read.csv(paste0(cutoff.dir, "!cells.csv"))$cell)
    for (cl in rownames(mad.clusters)) {
      cluster.cells <- as.character(subset(mad.cells, cluster == cl)$cell)
      mad.only.cells <- setdiff(cluster.cells, cutoff.cells)
      if (length(cluster.cells) >= 100 && length(mad.only.cells) / length(cluster.cells) >= 0.95) {
        mrk <- strsplit(as.character(mad.clusters[cl,]$markers), split=";")[[1]]
        write(paste(project, tissue, cl, as.character(mad.clusters[cl,]$cell.type), length(cluster.cells), round(length(mad.only.cells) / length(cluster.cells), 4), as.character(mad.clusters[cl,]$genes.mean), as.character(mad.clusters[cl,]$mito.mean), paste(mrk[1:min(25, length(mrk))], collapse=";", sep=""),  sep=","), file = "result.csv", append = TRUE)
      }
    }
  }
}
