...
for (project in dir(results.dir,  pattern="mc", recursive=FALSE)) {
  for (tissue in dir(paste0(results.dir, project, "/"), recursive=FALSE)) {
    ...
    source.dir <- paste0(results.dir, project, "/", tissue, "/filtered_cells_plots/joint/")
    ...
    for (cl in rownames(clusters)) {
      ...
      if (length(all.cells) >= 30 && length(mad.only.cells) / length(all.cells) >= 0) {
        ...
        write(paste(project, tissue, cl, as.character(clusters[cl,]$annotation), as.character(clusters[cl,]$cell.type), length(cluster.cells$barcodekey), round(length(mad.only.cells) / length(all.cells), 4), qc.criteria.line, as.character(clusters[cl,]$genes.median), as.character(clusters[cl,]$mito.median), ribo.median, paste(mrk[1:min(25, length(mrk))], collapse=";", sep=""),  sep=","), file = "result.csv", append = TRUE)
      }
    }
  }
  continue
